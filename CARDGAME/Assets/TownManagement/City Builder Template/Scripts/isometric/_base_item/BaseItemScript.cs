...
public class BaseItemScript : MonoBehaviour
{
	...
	private void OnDestroy()
	{
		if (connectedItems != null)
		{
			...
			{
                SceneManagerTM.instance.RemoveItem(connectedItems[index]);
			}
		}
	}
	...
	public void UpdateConnectedItems()
	{
		...
		if (this.itemData.name == "Tower")
		{
			...
			if (towerArcher == null)
			{
				towerArcher = SceneManagerTM.instance.AddItem(1502, true, ownedItem);
		...
		if (this.itemData.name == "BuilderHut" && SceneManagerTM.instance.gameMode == Common.GameMode.NORMAL)
		{
			if (SceneManagerTM.instance.selectedItem == this)
			{
			...
			if (builder == null)
			{
				builder = SceneManagerTM.instance.AddItem(3823, true, ownedItem);
